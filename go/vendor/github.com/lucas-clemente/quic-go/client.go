...
func SwitchPConn(currentQuicSession *Session, newConnection net.PacketConn) (error) {
	if currentQuicSession == nil {
		errors.New("Failed to switch connection: currentQuicSession: nil")
	}
	sess, ok := (*currentQuicSession).(*session)
	if !ok {
		errors.New("Failed to switch connection: currentQuicSession is not a session")
	}
	conn, ok := sess.conn.(*conn)
	if !ok {
		errors.New("Failed to switch connection: sess.conn is not a conn")
	}

	conn.mutex.Lock()
	conn.pconn = newConnection
	conn.mutex.Unlock()
	return nil
}
